...
class MlpMoeBlock(nn.Module):
  ...
  def create_router(self) -> nn.Module:
    ...
    return router_cls(dtype=self.dtype, name='Router', layer_num=self.layer_num, **router_kwargs)
  ...
  @nn.compact
  def __call__(self, inputs, patch_correspondence=None, return_router=False, learn_per=0., return_router_input=False):
    ...
    dispatcher, metrics = self.create_router()(inputs, patch_correspondence, return_router, learn_per, return_router_input)
...
class EncoderBlock(nn.Module):
  ...
  def __call__(self, inputs, patch_correspondence=None, return_router=False, learn_per=0., return_router_input=False):
    ...
    if patch_correspondence is None:
      if return_router or return_router_input:
        y = self.mlp_block(dtype=self.dtype, deterministic=self.deterministic, layer_num=self.layer_num)(y, return_router=return_router, return_router_input=return_router_input)
      else:
        y = self.mlp_block(dtype=self.dtype, deterministic=self.deterministic, layer_num=self.layer_num)(y)
    else:
      y = self.mlp_block(dtype=self.dtype, deterministic=self.deterministic, layer_num=self.layer_num)(y, patch_correspondence, return_router, learn_per, return_router_input)
    ...
    for block in range(self.num_layers):
      if block in moe_mlp_layers:
        x, metrics[f'encoderblock_{block}'] = encoder_block_cls(
            name=f'encoderblock_{block}', mlp_block=moe_mlp_cls, layer_num=block+1)(x, patch_correspondence, return_router, learn_per, return_router_input)
      else:
        x = encoder_block_cls(
            name=f'encoderblock_{block}', mlp_block=dense_mlp_cls, layer_num=block+1)(x)
...
class VisionTransformerMoe(nn.Module):
  ...
  @nn.compact
  def __call__(self, inputs, patch_correspondence=None, return_router=False, mixup_final_layer_weight=None, mixup_final_layer_permutation=None, learn_per=0., return_router_input=False):
    ...
    x, metrics = self.encoder_cls(
        name='Encoder', deterministic=self.deterministic, **encoder_kwargs)(x, patch_correspondence, return_router, learn_per, return_router_input)
    # Get a single vector representation of the full sequence.

    # Mixup final layer
    if mixup_final_layer_weight is not None:
      x = jax.lax.stop_gradient(mixup_final_layer_weight.reshape((-1, 1)) * x[mixup_final_layer_permutation]) + (1. - mixup_final_layer_weight.reshape((-1, 1))) * x
