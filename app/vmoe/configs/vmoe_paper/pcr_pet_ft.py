...
NUM_CLASSES = 37# Number of ILSVRC2012 classes.
...
def get_config():
  ...
  config.dataset.train = get_data_train_config(
      name=f'oxford_iiit_pet', split='train[:98%]',
      process=(
          #f'decode_jpeg_and_inception_crop({IMAGE_SIZE})|flip_lr|{pp_common}'))
          f'onehot({NUM_CLASSES}, inkey="label", outkey="labels")|decode_and_ssl_augment({IMAGE_SIZE})|value_range(-1, 1)|keep("image", "labels", "R")'),
      contrastive=True)
  ...
  config.dataset.test = get_data_eval_config(
      name=f'oxford_iiit_pet', split='test',
      process=f'decode|resize({IMAGE_SIZE})|{pp_common}')
  ...
  #config.evaluate.every_steps = 500
  # Description of the upstream model to fine-tune.
  config.description = 'ViT-S/32, E=8, K=2, Last 2, 300 Epochs'
  #config.description = 'ViT-S/32, E=8, K=1, Last 2, 300 Epochs'
  config.model = get_vmoe_config(config.description)

  config.model.encoder.moe.router.importance_loss_weight = 0.000#0.005
  config.model.encoder.moe.router.load_loss_weight = 0.000#0.005
  #config.model.encoder.moe.router.similarity_loss_weight = 0.01 #0.002
  #config.model.encoder.moe.router.onehot_loss_weight = 0.0000
  #config.model.encoder.moe.router.similarity_loss_input = "original"
  #config.model.encoder.moe.router.similarity_loss_type = "matrix_softmax_top1" # "matrix_softmax_top2"
  #config.model.encoder.moe.router.onehot_loss_type = "entropy"
  #config.model.encoder.moe.router.additional_losses = [("matrix_top1_diagonal", 2e-2), ("matrix_top1_offdiagonal", 1e-1), ("entropy", 0)]
  #config.model.encoder.moe.router.additional_losses = [("matrix_top1_diagonal", 5e-3), ("matrix_top1_offdiagonal", 5e-2), ("entropy", 0)]
  config.model.encoder.moe.router.additional_losses = [("matrix_top1_diagonal", 5e-3), ("matrix_top1_offdiagonal", 5e-2), ("entropy", 0)]
  #config.model.encoder.moe.router.additional_losses = [("jsd", 3e-4), ("two_hot", 5e-2), ("entropy", 0)]
  config.model.encoder.moe.router.teacher_student = False
  config.model.encoder.moe.router.similarity_loss_layer = [6, 8]
  #config.model.encoder.moe.router.barlow_lambd = 5. # 10
  config.model.encoder.moe.router.stop_grad = False
  config.patch_correspondence.cls_correspondence = False
  ...
  config.initialization = ml_collections.ConfigDict({
      ...
      'rules': [
          ...
          ('pre_logits/.*', ''),     # Do not restore the pre_logits params.
          # Resize positional embeddings.
          ('^(.*/pos_embedding)$', r'params/\1', 'vit_zoom'), # only gs
          ('^(.*)$', r'params/\1'), # only gs
      ],
      'raise_if_target_unmatched': False,
      'axis_resources_regexes': [('Moe/Mlp/.*', ('expert',))],
  ...
  # each device.
  config.num_expert_partitions = jax.local_device_count()#config.model.encoder.moe.num_experts
...
def get_data_train_config(name, split, process, contrastive=False):
  """Returns dataset parameters."""
  config = common.get_data_config(
      name=name, split=split, process=process, batch_size=BATCH_SIZE // (2 if contrastive else 1),
