...
def get_config():
  ...
  config.dataset.train = get_data_train_config(
      ...
      process=(
          ...
          f'onehot({NUM_CLASSES}, inkey="label", outkey="labels")|decode_jpeg_and_ssl_augment({IMAGE_SIZE})|value_range(-1, 1)|keep("image", "labels", "R")'),
      contrastive=True)
  ...
  config.dataset.test = get_data_eval_config(
      ...
      process=f'decode|resize({IMAGE_SIZE})|{pp_common}')
  config.dataset.test_a = get_data_eval_config(
      name='imagenet_a', split='test',
      process=f'decode|resize({IMAGE_SIZE})|{pp_common}')
  ...
  # Fine-tuning steps.
  config.train_steps = 5000 # 10_000
  ...
  # Fine tuning from local checkpoints.
  config.initialization = ml_collections.ConfigDict({
      'name': 'initialize_from_vmoe',
      'prefix':
          '/home/user/app/workdir/s_contrastive_cls_1e-2/ckpt_371589',
      'rules': [
          ('head', ''),              # Do not restore the head params.
          ('pre_logits/.*', ''),     # Do not restore the pre_logits params.
