...
PP_COMMON = f'value_range(-1,1)|onehot({NUM_CLASSES}, inkey="label", outkey="labels")|keep("image", "labels", "R")'
...
def get_config():
  ...
  config.model.encoder.moe.router.additional_losses = [("matrix_top1_diagonal", 5e-3), ("matrix_top1_offdiagonal", 5e-2)]
  #config.model.encoder.moe.router.additional_losses = [("matrix_top1_diagonal", 2e-2), ("matrix_top1_offdiagonal", 2e-2)]
  ...
  config.patch_correspondence.cls_correspondence = True
  # mixup final layer
  ...
  config.optimizer.learning_rate.peak_value /= 2048 / BATCH_SIZE
  config.optimizer.learning_rate.end_value /= 2048 / BATCH_SIZE

  config.train_epochs = common.get_num_epochs(config.description)
  # Mixup options.
  #config.mixup = common.get_mixup_config('light1') # TODO: ???
...
def get_data_params_train(
  ...
  # 通常のmixupを使用しない提案法
  randaug_ssl = common.get_randaug_contrastive(224, aug)
  process = f'{randaug_ssl}|{PP_COMMON}'
  config = common.get_data_config(
      name=name, split=split, batch_size=BATCH_SIZE // 2, process=process,
      shuffle_buffer=50_000, cache=None)

  # TODO: randaug, contrastive, R計算の実装
  #randaug = common.get_randaug(aug)
  #config = common.get_data_config(
  #    name=name, split=split, batch_size=BATCH_SIZE // 2, process=f'decode_jpeg_and_inception_crop(224, area_min=20)|flip_lr|{randaug}|onehot({NUM_CLASSES}, inkey="label", outkey="labels")',
  #    shuffle_buffer=50_000, cache=None,
  #    mixup_process='crop_contrastive(224)|value_range(-1,1)|keep("image", "labels", "R")')
...
def get_optimizer_params(description: str) -> ml_collections.ConfigDict:
  ...
  config.learning_rate.peak_value = 3e-3 / 2
