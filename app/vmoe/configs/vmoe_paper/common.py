...
DESCRIPTIONS_REGEX = re.compile(
    ...
    r'K=(?P<k>[0-9]+), '
    r'(?P<where>Every|Last|LastOdd) (?P<where_num>[0-9]+), '
...
def get_base_config() -> ml_collections.ConfigDict:
  ...
  config.save_checkpoint.wait_seconds = 20.0
  ...
  config.seed = 0
  config.patch_correspondence = ml_collections.ConfigDict()
  return config
...
def get_data_config(
  ...
  config.mixup_process = mixup_process
...
def get_randaug_contrastive(imgsize: int, aug: str, mixup: bool=False) -> str:
  """Returns a string representing the RandAugment op to use during preprocessing."""
  l, m = {
      'light0': (2, 0),
      'light1': (2, 10),
  ...
  #if mixup:
  #  return f'randaug_ssl_mixup({imgsize}, {l}, {m})'
  return f'decode_jpeg_and_randaug_ssl({imgsize}, {l}, {m},area_min=5,area_max=100,area2_min=5,area2_max=100)'

def get_vmoe_config(description: str, image_size: int,
                    num_classes: int, gap: bool = False) -> ml_collections.ConfigDict:
  ...
  variant = match.group('variant')
  variant_idx = ['S', 'B', 'L', 'H', 'T'].index(variant)
  patch_size = int(match.group('patch'))
  ...
  config.hidden_size = [512, 768, 1024, 1280, 384][variant_idx]
  ...
  config.encoder.num_layers = [8, 12, 24, 32, 8][variant_idx]
  config.encoder.mlp_dim = [2048, 3072, 4096, 5120, 1024][variant_idx]
  config.encoder.num_heads = [8, 12, 16, 16, 4][variant_idx]
  ...
  elif moe_where == 'LastOdd':
    config.encoder.moe.layers = tuple(
        range(0, config.encoder.num_layers, 2))[-moe_where_num:]
  else:
    raise ValueError(
