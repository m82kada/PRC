...
def get_dataset(
  ...
  Returns:
    A DatasetIterator.
  """
  if 'mixup_process' in extra_builder_kwargs:
    mixup_process = extra_builder_kwargs['mixup_process']
  else:
    mixup_process = ""
  extra_builder_kwargs = {k: v for k, v in extra_builder_kwargs.items() if k not in ['contrastive', 'mixup_process']}
  ...
  data = data.map(
      ...
      deterministic=False)
  if mixup_process != '':
    data = tf.data.Dataset.zip((data, data.shuffle(shuffle_buffer, seed=shuffle_seed))).map(
      map_func=lambda i, j: mixup(i, j),
      num_parallel_calls=num_parallel_calls,
      deterministic=False
    ).map(
      map_func=get_data_process_fn(mixup_process),
      num_parallel_calls=num_parallel_calls,
      deterministic=False)
...
def get_data_num_examples(config: ml_collections.ConfigDict) -> int:
  ...
  pipeline_keys = ('variant', 'batch_size', 'process', 'cache',
                   ...
                   'shuffle_buffer', 'contrastive', 'mixup_process')
...
def _mixup(alpha=0.2):
  beta = tf.compat.v1.distributions.Beta(alpha, alpha)
  def f(data1, data2):
    lamb = beta.sample(1)
    data1['image'] = (1 - lamb) * tf.cast(data1['image'], tf.float32) + lamb * tf.cast(data2['image'], tf.float32)
    data1['labels'] = (1 - lamb) * data1['labels'] + lamb * data2['labels']
    return data1
  return f
