...
def get_dataset_shape_dtype_struct(
  ...
  def fn(x):
    ...
    # Global shape
    if contrastive and len(shape) == 5: # image
      shape = (shape[0] * shape[1] * jax.process_count(),) + shape[2:]
    elif contrastive and len(shape) == 2: # labels
      shape = (2 * shape[0] * jax.process_count(),) + shape[1:]
    else:
