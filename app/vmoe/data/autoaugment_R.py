...
def rotate(image: tf.Tensor, R: tf.Tensor, degrees: float) -> Tuple[tf.Tensor, tf.Tensor]:
  ...
  _R = tf.stack([transforms[0,:3], transforms[0,3:6], tf.concat([transforms[0,6:], [1.]], axis=0)])
  return from_4d(image, original_ndims), tf.matmul(_R, R)
...
def cutout(image: tf.Tensor, R: tf.Tensor, pad_size: int, replace: int = 0) -> Tuple[tf.Tensor, tf.Tensor]:
...
def solarize(image: tf.Tensor, R: tf.Tensor, threshold: int = 128) -> Tuple[tf.Tensor, tf.Tensor]:
  """Solarize the input image(s)."""
  # For each pixel in the image, select the pixel
  # if the value is less than the threshold.
  # Otherwise, subtract 255 from the pixel.
  return tf.where(image < threshold, image, 255 - image), R


def solarize_add(image: tf.Tensor,
                 R: tf.Tensor,
                 addition: int = 0,
                 threshold: int = 128) -> Tuple[tf.Tensor, tf.Tensor]:
  ...
  return tf.where(image < threshold, added_image, image), R
...
def color(image: tf.Tensor, R: tf.Tensor, factor: float) -> Tuple[tf.Tensor, tf.Tensor]:
  """Equivalent of PIL Color."""
  degenerate = grayscale(image)
  return blend(degenerate, image, factor), R


def contrast(image: tf.Tensor, R: tf.Tensor, factor: float) -> Tuple[tf.Tensor, tf.Tensor]:
...
def brightness(image: tf.Tensor, R: tf.Tensor, factor: float) -> Tuple[tf.Tensor, tf.Tensor]:
  """Equivalent of PIL Brightness."""
  degenerate = tf.zeros_like(image)
  return blend(degenerate, image, factor), R


def posterize(image: tf.Tensor, R: tf.Tensor, bits: int) -> Tuple[tf.Tensor, tf.Tensor]:
  """Equivalent of PIL Posterize."""
  shift = 8 - bits
  return tf.bitwise.left_shift(tf.bitwise.right_shift(image, shift), shift), R


def wrapped_rotate(image: tf.Tensor, R: tf.Tensor, degrees: float, replace: int) -> Tuple[tf.Tensor, tf.Tensor]:
  """Applies rotation with wrap/unwrap."""
  image, R = rotate(wrap(image), R, degrees=degrees)
  return unwrap(image, replace), R


def translate_x(image: tf.Tensor, R: tf.Tensor, pixels: int, replace: int) -> Tuple[tf.Tensor, tf.Tensor]:
  """Equivalent of PIL Translate in X dimension."""
  image = translate(wrap(image), [-pixels, 0])
  _R = tf.reshape(tf.stack([
    1., 0., 0.,
    0., 1., -pixels,
    0., 0., 1.
  ]), [3, 3])
  return unwrap(image, replace), tf.matmul(_R, R)


def translate_y(image: tf.Tensor, R: tf.Tensor, pixels: int, replace: int) -> Tuple[tf.Tensor, tf.Tensor]:
...
def shear_x(image: tf.Tensor, R: tf.Tensor, level: float, replace: int) -> Tuple[tf.Tensor, tf.Tensor]:
...
def shear_y(image: tf.Tensor, R: tf.Tensor, level: float, replace: int) -> Tuple[tf.Tensor, tf.Tensor]:
...
def autocontrast(image: tf.Tensor, R: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
...
def sharpness(image: tf.Tensor, R: tf.Tensor, factor: float) -> Tuple[tf.Tensor, tf.Tensor]:
  ...
  return blend(result, orig_image, factor), R


def equalize(image: tf.Tensor, R: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
...
def invert(image: tf.Tensor, R: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
...
class RandAugment(ImageAugment):
      ...
      for (i, op_name) in enumerate(self.available_ops):
        ...
        branch_fns.append((
            ...
            lambda selected_func=func, selected_args=args: selected_func(
                image, R, *selected_args)))
        # pylint:enable=g-long-lambda

      aug_image, _R = tf.switch_case(
          branch_index=op_to_select,
          branch_fns=branch_fns,
          default=lambda: (tf.identity(image), tf.identity(R)))  # pylint: disable=cell-var-from-loop
  ...
  def distort(self, image: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
    """See base class."""
    return self._distort_common(image)
