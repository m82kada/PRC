...
def make_create_train_state_fn(
    *,
    ...
    patch_correspondence_shape: Optional[Tuple[int, ...]],
  ...
  def initialize():
    ...
    if patch_correspondence_shape:
      patch_correspondence = jnp.zeros(patch_correspondence_shape, dtype=jnp.int32)
      patch_correspondence = partitioning.with_sharding_constraint(patch_correspondence, input_axis_resources)
    else:
      patch_correspondence = None
    variables = model.init(rngs, inputs, patch_correspondence)
...
def get_train_steps_and_epochs(
  ...
  if not train_steps:
    if contrastive:
      train_steps = int(np.ceil(train_epochs * train_examples / (train_batch_size * 2)))
    else:
...
def train_step(
    ...
    images: Array,
    labels: Array,
    patch_correspondence: Optional[Array],
    loss_fn: Callable[[Array, Array], Array],
    microsteps: Optional[int] = None,
    summarizer: Optional[TreeSummarizer] = None,
    mixup_final_layer_weight: Optional[Array] = None,
    mixup_final_layer_permutation: Optional[Array] = None,
) -> Tuple[TrainState, Mapping[str, Any]]:
  """Performs one update step of the given TrainState object ."""

  @functools.partial(jax.grad, has_aux=True)
  def compute_grads_and_metrics(params, images, labels, rngs, patch_correspondence):
    rngs, next_rngs = utils.tree_rngs_split(state.rngs)
    logits, metrics = state.apply_fn({'params': params}, images, patch_correspondence, mixup_final_layer_weight=mixup_final_layer_weight, mixup_final_layer_permutation=mixup_final_layer_permutation, rngs=rngs)
  ...
  grads, (next_rngs, metrics) = compute_grads_and_metrics(
      state.params, images, labels, state.rngs, patch_correspondence)
...
def train_and_evaluate(config: ml_collections.ConfigDict, workdir: str,
  ...
  train_steps, train_epochs = get_train_steps_and_epochs(
      ...
      contrastive=config.dataset.train.contrastive)
  logging.info(
  ...
  dataset_element_shape_dtype = pjit_utils.get_dataset_shape_dtype_struct(
      datasets['train'], contrastive=config.dataset.train.contrastive)

  if config.dataset.train.contrastive:
    #_window_shape = jax.ShapeDtypeStruct(dataset_element_shape_dtype['windows'].shape[1:], jnp.int32)
    #patch_correspondence_dtype = jax.ShapeDtypeStruct((dataset_element_shape_dtype['windows'].shape[0],
    #                                                   jax.eval_shape(utils.window_to_patch_correspondence, _window_shape).shape[0],
    #                                                   2), jnp.int32)
    patch_correspondence_dtype = jax.ShapeDtypeStruct(utils.get_patch_correspondences_shape(dataset_element_shape_dtype['R'].shape, cls_correspondence=config.patch_correspondence.cls_correspondence, image_size=config.model.image_size, patch_size=config.model.patch_size[0]), jnp.int32)
    train_state_initialize_fn = make_create_train_state_fn(
        ...
        input_shape=dataset_element_shape_dtype['image'].shape,
        patch_correspondence_shape=patch_correspondence_dtype.shape,
        input_axis_resources=dataset_element_shape_dtype['image'].sharding,
    ...
    train_state_initialize_fn = make_create_train_state_fn(
        ...
        optimizer_config=config.optimizer,
        input_shape=dataset_element_shape_dtype['image'].shape,
        patch_correspondence_shape=None,
        input_axis_resources=dataset_element_shape_dtype['image'].sharding,
        train_steps=train_steps,
        extra_rng_keys=tuple(config.get('extra_rng_keys', [])),
        seed=config.get('seed', 0))
  ...
  with metric_writers.ensure_flushes(writer):
    ...
    if config.dataset.train.contrastive:
      train_step_pjit = train_step_pjit.lower(
          ...
          dataset_element_shape_dtype['labels'],
          patch_correspondence_dtype).compile()
    else:
      train_step_pjit = train_step_pjit.lower(
          train_state,
          dataset_element_shape_dtype['image'],
          dataset_element_shape_dtype['labels'], None).compile()
    t1 = time.time()
    ...
    for step, batch in zip(range(init_step + 1, train_steps + 1), tr_iter):
      ...
      with jax.profiler.StepTraceAnnotation('train', step_num=step):
        if config.dataset.train.contrastive:
          ...
          patch_correspondence = utils.generate_patch_correspondences(batch['R'], **config.patch_correspondence, image_size=config.model.image_size, patch_size=config.model.patch_size[0])
          # TODO: image size, patch size (+ debug)
          image = jnp.array(batch['image']).reshape(shape)
          label = jnp.stack([batch['labels'], batch['labels']], axis=1).reshape((shape[0], -1))
          #train_state, metrics = train_step_pjit(train_state, image, label, patch_correspondence, step / train_steps)  # decay
          train_state, metrics = train_step_pjit(train_state, image, label, patch_correspondence)
        else:
    ...
    jax.profiler.save_device_memory_profile(f"{workdir}/memory.prof")
...
def wrap_train_step_with_mixup_final_layer(
    train_step_fn: TrainStepFn,
    alpha: float = 2.
) -> TrainStepFn:
  """Wraps a train step function with mixup.
  ...
  def new_train_step_fn(
    ...
    # Apply mixup on images and labels.
    mixup_rng = rngs.pop('mixup')
    mixup_rng, mixup_permutation_key = jax.random.split(mixup_rng)
    mixup_rng, next_mixup_rng = jax.random.split(mixup_rng)
    
    # TODO: Calculate on each device independetly
    # Reshape images and labels to perform mixup on each device independently,
    # to avoid communication among devices due to mixup's jnp.roll().
    ...
    mixup_final_layer_weight = jax.random.beta(mixup_rng, a=alpha, b=alpha, shape=(labels.shape[0]))
    mixup_final_layer_permutation = jax.random.permutation(mixup_permutation_key, labels.shape[0])

    labels = mixup_final_layer_weight.reshape((-1, 1)) * labels[mixup_final_layer_permutation] + (1. - mixup_final_layer_weight.reshape((-1, 1))) * labels
    #labels = labels.reshape((-1,) + labels.shape[2:])

    # Call the given `train_step_fn` passing the state without mixup rngs, and
    # the modified images and labels.
    state = state.replace(rngs=flax.core.freeze(rngs) if frozen_rngs else rngs)
    state, metrics = train_step_fn(state, images, labels, *args, **kwargs, mixup_final_layer_weight=mixup_final_layer_weight, mixup_final_layer_permutation=mixup_final_layer_permutation)
    # Add the mixup PRNGKey back to the state.
    rngs = flax.core.unfreeze(state.rngs) if frozen_rngs else state.rngs
    rngs['mixup'] = next_mixup_rng
    state = state.replace(rngs=flax.core.freeze(rngs) if frozen_rngs else rngs)
